...
App.controller('SearchEntityController', ['SearchEntityService', '$scope', '$filter', 'toastr','$uibModal', function (SearchEntityService, scope, filter, MessageService, $modal) {
    ...
    self.validateRec=function(rowData, meta_Info, msg, rowId) {
        for (var col in meta_Info){
            ...
            var rowIdVal=rowId==undefined? "":" (Row#" + (rowId+1) + ")";
            var oVal=rowData[col];
            if (rowData['objectState']==ObjectState.Deleted) {break;}  //no need to validate deleted record
            if (oVal instanceof Array) break; //Root's child entity field, ignore it
            if (colObj.ColType == "Number" && (isNaN(oVal) || oVal==="")) {
                msg.push("'" + col + "'" + " is not a valid number." + rowIdVal);
            }
            if (colObj.Scale>0){
                if (parseInt(oVal)>0 && (parseInt(oVal)+"").length>colObj.Precision-colObj.Scale){
                    msg.push("'"+colObj.ColDesc+"'" +" integer portion two long."+ rowIdVal);
                }
            }
            // don't check length for Date or Boolean columns
            if (colObj.ColType!=="Date" && colObj.ColType!=="Boolean"){
                if((!isEmpty(colObj.ColLength)) && colObj.ColType!=="Date" && (oVal+"").length>colObj.ColLength){
                    msg.push("'"+colObj.ColDesc+"'" +" length exceeds "+colObj.ColLength+ rowIdVal);
                }
                if((!isEmpty(colObj.MaxLength)) && colObj.ColType!=="Date" && !isEmpty(oVal) && (oVal+"").length>colObj.MaxLength){
                    msg.push("'"+colObj.ColDesc+"'" +" length exceeds "+colObj.MaxLength+ rowIdVal);
                }
                if((!isEmpty(colObj.MinLength)) && ( isEmpty(oVal) || (oVal+"").length<colObj.MinLength)){
                    msg.push("'"+colObj.ColDesc+"'" +" should be longer than "+colObj.MinLength+ rowIdVal);
                }
            }

            if((!isEmpty(colObj.MinValue)) && oVal<colObj.MinValue){
                msg.push("'"+colObj.ColDesc+"'" +" should be greater than "+colObj.MinValue+ rowIdVal);
            }
            if((!isEmpty(colObj.MaxValue)) && oVal>colObj.MaxValue){
                msg.push("'"+colObj.ColDesc+"'" +" should be less than "+colObj.MaxValue+ rowIdVal);
            }
            if((!isEmpty(colObj.Nullable)) && !colObj.Nullable && isEmpty(oVal)){
                msg.push("'"+colObj.ColDesc+"'" +" should not be empty."+ rowIdVal);
            }
            if(!isEmpty(colObj.ColRegex)){
                var regEx = new RegExp(colObj.ColRegex);
                if (!regEx.test(oVal)) {
                    msg.push("Value of '"+colObj.ColDesc+"': '" +oVal+"' is not in correct format."+ rowIdVal);
                }
            }
        }
        return msg;
    ...
    //self.hasBufferedChanges=function(){
    //    if (self.entities.length!==self.savedEntities.length) return true;
    //    for (var i=0;i<self.entities.length;i++){
    //        for (var j=0; j<self.entities[i].length;j++){
    //            if (self.entities[i][j]!=self.savedEntities[i][j])
    //                return true;
    //        }
    //    }
    //    return false;
