...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		add_rowNum = function (pos,irow,pG,rN) {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				rowData.push("");
				if( ni ) {
					rowData.push( add_rowNum(0,j,ts.p.page,ts.p.rowNum) );
				}
				if( gi ) {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				rowData.push("");
				if( ni ) {
					rowData.push( add_rowNum(0,i,ts.p.page,ts.p.rowNum) );
				}
				if( gi ){
...
$.jgrid.extend({
	...
	add_rowData : function(rowid,rdata,pos,src) {
		if(!pos) {pos = "last";}
...
$.jgrid.extend({
								...
								if(postdata[oper] === opers.addoper ) {
									...
									if(rp_ge[$t.p.id].reloadAfterSubmit) {
										...
										if($t.p.treeGrid === true){
											...
											$($t).jqGrid("add_rowData",ret[2],postdata,p.addedrow);
										}
									}
	...
	FormToGrid : function(rowid, formid, mode, position){
		return this.each(function() {
			...
			if(mode==='add') {$($t).jqGrid("add_rowData",rowid,griddata, position);}
...
//Editing
	...
	add_row : function ( p ) {
		p = $.extend(true, {
			...
			useDefValues : true,
			useFormatter : false,
			add_rowParams : {extraparam:{}}
		},p  || {});
		return this.each(function(){
			if (!this.grid ) { return; }
			var $t = this;
			var bfar = $.isFunction( p.beforeadd_row ) ?	p.beforeadd_row.call($t,p.add_rowParams) :  undefined;
			if( bfar === undefined ) {
				bfar = true;
			}
			if(!bfar) { return; }
			...
			$($t).jqGrid('add_rowData', p.rowID, p.initdata, p.position);
			...
			if(p.useFormatter) {
				...
				oper = opers.oper;
				p.add_rowParams.extraparam[oper] = opers.addoper;
				$($t).jqGrid('editRow', p.rowID, p.add_rowParams);
	...
	inlineNav : function (elem, o) {
		o = $.extend(true,{
			...
			addParams : {add_rowParams: {extraparam: {}}},
			editParams : {},
					...
					onClickButton : function () {
						...
						if(sr) {
							...
							if($("#"+$.jgrid.jqID(sr), "#"+gID ).hasClass("jqgrid-new-row")) {
								o.addParams.add_rowParams.extraparam[oper] = opers.addoper;
								tmpParams = o.addParams.add_rowParams;
							} else {
			...
			if(o.cancel) {
				$($t).jqGrid('navButtonAdd', elem,{
					...
					onClickButton : function () {
						...
						if(sr) {
							if($("#"+$.jgrid.jqID(sr), "#"+gID ).hasClass("jqgrid-new-row")) {
								cancelPrm = o.addParams.add_rowParams;
							}
	...
	addChildNode : function( nodeid, parentid, data, expandData ) {
		...
		if(data) {
			...
			if( parentid === null || $($t).jqGrid("isNodeLoaded",parentdata) || leaf ) {
					$($t).jqGrid('add_rowData', nodeid, data, method, rowind);
					...
					start : function (ev, ui) {
						...
						for (i=0;i<$.data($t,"dnd").connectWith.length;i++){
							if($($.data($t,"dnd").connectWith[i]).jqGrid('getGridParam','reccount') === 0 ){
								$($.data($t,"dnd").connectWith[i]).jqGrid('add_rowData','jqg_empty_row',{});
							}
						}
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if(ui.helper.dropped) {
							...
							$("#"+$.jgrid.jqID(this.id)).jqGrid('add_rowData',grid,getdata,opts.droppos);
						}
...
jQuery(selector).each(function() {
	...
	for (a = 0; a < data.length; a++) {
		...
		jQuery(this).jqGrid("add_rowData",id, data[a]);
	}
